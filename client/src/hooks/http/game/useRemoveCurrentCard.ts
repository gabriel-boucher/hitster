...
export default function useRemoveCurrentCard() {
    ...
    return useCallback(async () => {
        try {
            await removeCurrentCard(roomId, playerId);
        } catch (error) {
...
async function removeCurrentCard(gameId: RoomId, playerId: PlayerId): Promise<EventResponse<GameState>> {
    const response = await axios.post(
        `${HTTP_SERVER_URL}/api/game/${GameHttpEvents.REMOVE_CURRENT_CARD}`,
        {
            gameId,
            playerId,
        }
