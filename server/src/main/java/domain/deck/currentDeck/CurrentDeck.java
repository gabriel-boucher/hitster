package domain.deck.currentDeck;

import domain.deck.currentDeck.exception.*;
...
public class CurrentDeck {
    ...
    public void addCardToDeck(Card card) {
        ...
        card.setStatus(ItemStatus.ACTIVE_IN_CURRENT_DECK);
        currentItems.replaceAll(item -> item.equals(card) ? card : item);
    }
    ...
    public void returnCardToStack(Card card) {
        if (card.getStatus() != ItemStatus.BOARD) {
            throw new CardAlreadyInStackException();
        }

        card.setStatus(ItemStatus.UNUSED);
    }

    public Optional<Token> moveCardInDeck(Card card, int position) {
        currentItems.remove(card);
        ...
        for (Moveable item : currentItems) {
            ...
            if (item instanceof Token token) {
                ...
                if (currentCard.getDate() <= card.getDate()) {
                    // We found the correct slot. Is there an id here?
