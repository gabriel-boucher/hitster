...
import { GameInterface, PlayerInterface } from "../../../shared/Interfaces";
import {
  gameStates,
  initialGameState,
  PLAYERS_IMG,
...
export default function useJoinRoom(
  ...
  return async function joinRoom(this: Socket, roomId: string) {
    const socket = this;

    if (!rooms[roomId]) {
      rooms[roomId] = structuredClone(initialGameState);
      codeToToken[roomId] = await getAccessToken(roomId);;
      console.log(await searchPlaylists(codeToToken[roomId]));
    }
